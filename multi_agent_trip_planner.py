...
def ask_question(question):
    # Only ask the question if it's not already the last message
...
current_stage = st.session_state.chat_stage
...
if current_stage == "collecting":
    ...
    elif "departure" not in user_data:
...
if user_input:
    ...
    if current_stage == "collecting":
        if "destination" not in user_data:
            user_data["destination"] = user_input
        elif "days" not in user_data:
            try:
                user_data["days"] = int(user_input)
            except ValueError:
                st.session_state.messages.append({"role": "assistant", "content": "Please enter a valid number for the days."})
        elif "budget" not in user_data:
            user_data["budget"] = user_input.lower().strip()
        elif "departure" not in user_data:
            user_data["departure"] = user_input
